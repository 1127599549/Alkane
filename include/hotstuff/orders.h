...
#ifndef _THEMIS_ORDERS_H
#define _THEMIS_ORDERS_H
...
class MergeOrder{
    ...
    std::unordered_map<ReplicaID, bool> visited;
    void reset_visited();
    // void create_MergeOrder();
    // void create_order_by_finish_time(uint256_t vertex);

    public:
    MergeOrder(std::unordered_map<ReplicaID, std::vector<uint256_t>> const&);
    std::unordered_map<ReplicaID, std::vector<uint256_t>> get_merge_order();
};
