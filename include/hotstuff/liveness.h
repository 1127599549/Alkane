...
class PMRoundRobinProposer: virtual public PaceMaker {
    ...
    void do_new_consensus(int x, const std::vector<uint256_t> &cmds) {
        ...
        // auto blk = hsc->on_propose(cmds, get_parents(), bytearray_t()); //Us
        std::unordered_map<ReplicaID, std::vector<uint256_t>> orders;
        auto blk = hsc->on_propose(orders,get_parents(),bytearray_t()); //Us
        pm_qc_manual.reject();
    ...
    void stop_rotate() {
        ...
        if (proposer == hsc->get_id())
        {
            ...
            hs->get_tcall().async_call([this, hs](salticidae::ThreadCall::Handle &) {
                ...
                // Omit the step to stop rotating for generating new commands
                ...
                // }
                // do_new_consensus(0, cmds);       // TODO: Us
            });
        }
    }
